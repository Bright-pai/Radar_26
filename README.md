...
TCP接收信息，端口8001，当有8001端口数据接收后，开始解析，0x0A01为对方机器人位置，顺序是
对方英雄机器人位置 x 轴坐标，单位：cm  两字节大小（后续都是）
对方英雄机器人位置 y 轴坐标，单位：cm
对方工程机器人位置 x 轴坐标，单位：cm
对方工程机器人位置 y 轴坐标，单位：cm
对方 3 号步兵机器人位置 x 轴坐标，单位：cm
对方 3 号步兵机器人位置 y 轴坐标，单位：cm
对方 4 号步兵机器人位置 x 轴坐标，单位：cm
对方 4 号步兵机器人位置 y 轴坐标，单位：cm
对方空中机器人位置 x 轴坐标，单位：cm
对方空中机器人位置 y 轴坐标，单位：cm
对方哨兵机器人位置 x 轴坐标，单位：cm
对方哨兵机器人位置 y 轴坐标，单位：cm
当这个有数值时就需要填充到0x0305的对方机器人坐标中进行转发
SOF          0XA5
data_length  24
seq
crc8
...
0X0A02是对方机器人血量，顺序是
对方 1 号英雄机器人血量，若该机器人未上场或者被罚下，则血量为 0，下文同理，大小均为两字节
对方 2 
0
SOF          0XA5
data_length  12
...
0x0A02~0x0A05的数据是填充到0x0301这一id下的0x0206的子内容id的66字节大小的数据转发出去的

接下来是8002,8003,8004端口接收到的数据，代表这是几级干扰波。从（8002端口）1开始，最高为（8004）3。
密钥解析如下，是六个字节的ASCII 码编码的字母或数字。
SOF          0XA5
data_length  6
seq
crc8
...
data 
crc16
密钥的发送逻辑：不断接收和解析，根据0x020E解析的数据己方加密等级，即几级通过0x0121发送几次（第一次发送的是8002端口解析的数据，第二次8003端口，以此类推）.0x121的发送数据端是是否触发双倍易伤，密钥（1改自己的，2发送破解的密钥），六字节的密钥。